// Version:110P3
function md5_pwd(pwd, key) {
	var md5one = md5(pwd, key);
	var md5two = md5(md5one, "YW2sg2T");
	var md5three = md5(md5one, "Z0Hi1oOps3G8");
	// to uppercase
	...
	for (var i = 0; i < source.length; ++i) {
		if (isNaN(source[i])) {
			var str = "b23Mc9vbI?A1c2tc1H!?r?HJZ0PK1qXo!G6bS";
...
function generate_password(pwd, key) {
	if (pwd && key) {
		var fl_pwd = md5_pwd(pwd, key);
		var hash = fl_pwd[1];
		console.assert(hash.length === 32, "md5_pwd output length not equal to 32");
